...
MODEL_DIR_V2 = os.getenv('COSYVOICE_MODEL_DIR_V2', 'pretrained_models/CosyVoice2-0.5B')
MODEL_DIR_V3 = os.getenv('COSYVOICE_MODEL_DIR_V3', 'pretrained_models/Fun-CosyVoice3-0.5B')
...
def get_cosyvoice3():
    """获取 CosyVoice3 实例（最优先使用）"""
    global _cv3
    if not MODEL_DIR_V3:
        return None
    if _cv3 is None:
        logger.info('Initializing CosyVoice3 with model_dir=%s', MODEL_DIR_V3)
        try:
            _cv3 = CosyVoice3(MODEL_DIR_V3)
            logger.info('Loaded CosyVoice3 model')
        except (ValueError, FileNotFoundError) as e:
            logger.warning(f'CosyVoice3 not available: {e}')
            return None
        except Exception as e:
            logger.warning(f'Failed to load CosyVoice3: {e}')
            return None
    return _cv3
...
def get_primary_model():
    """
    获取主模型：优先使用 CosyVoice3，其次 CosyVoice2，不可用时降级到 CosyVoice
    """
    cv3 = get_cosyvoice3()
    if cv3 is not None:
        return cv3
    ...
    if cv2 is not None:
        return cv2
    logger.info('CosyVoice2/3 not available, falling back to CosyVoice')
    return get_cosyvoice()


def _model_type_str(cv) -> str:
    if isinstance(cv, CosyVoice3):
        return 'CosyVoice3'
...
async def voice_create_and_preview(name: str = Form(),
    ...
    # 模式2：声音描述生成（需要 CosyVoice-Instruct 模型）
        ...
        if isinstance(used_model, CosyVoice3):
            model_type = 'CosyVoice3'
        elif isinstance(used_model, CosyVoice2):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model_dir_v2', type=str, default=MODEL_DIR_V2)
    parser.add_argument('--model_dir_v3', type=str, default=MODEL_DIR_V3)
    ...
    os.environ['COSYVOICE_MODEL_DIR_V2'] = args.model_dir_v2
    if args.model_dir_v3:
        os.environ['COSYVOICE_MODEL_DIR_V3'] = args.model_dir_v3
